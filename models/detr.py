...
class KeypointsSetCriterion(nn.Module):
    """ This class computes the loss for DETR.
    The process happens in two steps:
        1) we compute hungarian assignment between ground truth boxes and the outputs of the model
        2) we supervise each pair of matched ground-truth / prediction (supervise class and box)
    ...
    def __init__(self, matcher, weight_dict, eos_coef, losses):
        ...
        self.eos_coef = eos_coef
        self.losses = losses
        empty_weight = torch.tensor([1, eos_coef])
        self.register_buffer('empty_weight', empty_weight)
    ...
    def loss_keypoints(self, outputs, targets, indices, num_boxes):
        assert 'pred_confidence' in outputs and 'pred_location' in outputs

        idx = self._get_src_permutation_idx(indices)

        src_conf = outputs['pred_confidence'][idx]
        tgt_conf = torch.cat([t['keypoints'][i, :, 2] for t, (_, i) in zip(targets, indices)], dim=0)
        loss_conf = F.l1_loss(src_conf, tgt_conf, reduction='none')

        src_loc = outputs['pred_location'][idx]
        src_loc = src_loc.view(-1, src_loc.size(1) // 2, 2)
        tgt_loc = torch.cat([t['keypoints'][i, :, :2] for t, (_, i) in zip(targets, indices)], dim=0)

        loss_loc = loss_conf.new_zeros(len(tgt_conf))
        for idx, (conf, src, tgt) in enumerate(zip(tgt_conf, src_loc, tgt_loc)):
            num_keypoints = (conf > 0).sum()
            assert num_keypoints > 0
            loss = F.l1_loss(src[conf > 0].flatten(), tgt[conf > 0].flatten(), reduction='none')
            loss_loc[idx] = loss.sum() / num_keypoints

        return dict(loss_conf=loss_conf.sum() / num_boxes,
                    loss_loc=loss_loc.sum() / num_boxes)
    ...
    def forward(self, outputs, targets):
        ...
        # Compute the average number of target boxes accross all nodes, for normalization purposes
        num_boxes = sum(len(t["labels"]) for t in targets)  # TODO(mitya52): rename boxes to poses?
        num_boxes = torch.as_tensor([num_boxes], dtype=torch.float, device=next(iter(outputs.values())).device)
        if is_dist_avail_and_initialized():
            torch.distributed.all_reduce(num_boxes)
        num_boxes = torch.clamp(num_boxes / get_world_size(), min=1).item()
